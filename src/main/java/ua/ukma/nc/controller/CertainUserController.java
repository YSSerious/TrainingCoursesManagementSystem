...
public class CertainUserController {
    ...
    //G, T -- types of available reviews
    //B stands for 'BOTH'; E 'ERROR'
    @RequestMapping(value = "/ajax/get/hr_review", method = RequestMethod.GET)
    @ResponseBody
    public JsonWrapperHRAvailableProject getProjectForHRReview(@RequestParam("studentId") Long studentId, @RequestParam("projectId") Long projectId) {
        JsonWrapperHRAvailableProject wrapper = new JsonWrapperHRAvailableProject();
        wrapper.setProject(new ProjectDto(projectService.getById(projectId)));
        boolean general = !finalReviewService.existsForProject(studentId, projectId, "G");
        boolean technical = !finalReviewService.existsForProject(studentId, projectId, "T");
        System.out.println(general + "_" + technical);
        if (general && technical)
            wrapper.setAvailable('B');
        else if (general) wrapper.setAvailable('G');
        else if (technical) wrapper.setAvailable('T');
        else wrapper.setAvailable('E');
        return wrapper;
    }

    @RequestMapping(value = "/ajax/post/hr_review", method = RequestMethod.POST)
    @ResponseBody
    public String createHRreview(Principal principal, @RequestParam("type") String type, @RequestParam("comment") String ccomment, @RequestParam("studentId") Long studentId, @RequestParam("projectId") Long projectId) {
        FinalReview finalHRreview = new FinalReviewImpl();
        finalHRreview.setCommentary(ccomment);
        ...
        finalHRreview.setProject(projectService.getById(projectId));
        finalHRreview.setStudent(userService.getById(studentId));
        if(!finalReviewService.existsForProject(studentId,projectId,type)) {
